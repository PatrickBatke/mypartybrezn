...
jQuery( function ( $ ) {
	...
	// jQuery("select#ocdi__demo-import-files").imagepicker({
 //      hide_select:  true,
 //    });
 	
 	var demoList = new List('mp-demo-list', { 
	  valueNames: ['demo__name'], 
	});

 	var demoLimit = 20;
	$(".demo-item").slice(0, demoLimit).show().addClass("visible");
    $(".load-more-demo").on('click', function (e) {
        e.preventDefault();
        if( $(".demo-item:hidden").length ){
        	$(".demo-item:hidden").slice(0, demoLimit).show().addClass("visible");
        } else {
        	$(this).attr({disabled: 'disabled'}).html("No more demo");
        }
    });
	...
	$( '.js-ocdi-import-data' ).on( 'click', function () {
		...
		var demoId = parseInt( $(this).data('demo-id') );
	...
	$(".close-demo-popup").on( 'click', function () {
		$('.demo-install-overlay').fadeOut();
	} );
