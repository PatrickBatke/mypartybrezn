...
require PT_OCDI_PATH . 'inc/class-ocdi-toptions-importer.php';
...
class PT_One_Click_Demo_Import {
	...
	protected function __construct() {
		...
		// Actions.
		add_action( 'admin_menu', array( $this, 'create_plugin_page' ), 10 );
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import' , 'tokoo-vitamins' ),
				'menu_title'  => esc_html__( 'Import Demo Data' , 'tokoo-vitamins' ),
				'capability'  => 'manage_options',
				'menu_slug'   => 'tokoo-demo-import',
			)
		);
	...
	<div class="ocdi  wrap">
		...
		if ( ini_get( 'safe_mode' ) ) {
			printf(
				esc_html__( '%sWarning: your server is using %sPHP safe mode%s. This means that you might experience server timeout errors.%s', 'tokoo-vitamins' ),
		...
		<div class="tokoo-demo-importer-intro">
			<h2>Welcome to the Demo Importer Page</h2>
			<div class="row">
				<div class="col-1 importer-intro">
					<p >
						<?php esc_html_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch. When you import the data, the following things might happen:', 'tokoo-vitamins' ); ?>
					</p>
				</div>
				<div class="col-2">
					<h3><?php esc_html_e( 'Demo Importer Reminder.', 'tokoo-vitamins' ); ?></h3>
					<ul class="demo-notices">
						<li><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', 'tokoo-vitamins' ); ?></li>
						<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'tokoo-vitamins' ); ?></li>
						<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', 'tokoo-vitamins' ); ?></li>
						<li><?php esc_html_e( 'Please click "Import Demo Data" button only once and wait, it can take a couple of minutes.', 'tokoo-vitamins' ); ?></li>
					</ul>
				</div>
			</div>
		</div>
		...
		<?php if ( 0 < count( $this->import_files ) ) : ?>
			...
			<div id="mp-demo-list" class="ocdi__multi-select-import">
				<label for="mp-demo-search">Search from <?php echo count( $this->import_files ) ?> Demos

					<input id="mp-demo-search" class="search" type="text" placeholder="Type keyword to filter demo">

				</label>
				<ul class="demo-picker list">
					<?php foreach ( $this->import_files as $index => $import_file ) : ?>
						<li class="demo-item">
							<div class="inner-content">
								<figure class="demo__image">
									<img src="<?php echo esc_url( $import_file['import_preview_image_url'] ); ?>" alt="">
								</figure>
								<div class="demo__detail">
									<h2 class="demo__name"><?php echo esc_html( ucfirst( $import_file['import_file_name'] ) ); ?></h2>
									<div class="demo__notice"><?php echo esc_html( $import_file['import_notice'] ); ?></div>
									<div class="demo__action">
										<button class="ocdi__button  button-primary  js-ocdi-import-data" data-demo-id="<?php echo esc_attr( $index ); ?>"><?php esc_html_e( 'Import Demo Data', 'tokoo-vitamins' ); ?></button>
										<a href="<?php echo esc_url( $import_file['import_demo_url'] ); ?>" target="_blank" class="button-secondary">View Demo</a>
									</div>
								</div>
							</div>
						</li>
				...
				<a href="#" class="button primary load-more-demo">Load More</a>
			</div>
		<?php endif; ?>

		<div class="demo-install-overlay">
			<div class="demo-install-popup">
				<button class="close-demo-popup">x</button>
				<div class="ocdi__ajax-loader  js-ocdi-ajax-loader">
					<span class="spinner"></span> 
					<h2><?php esc_html_e( 'Please wait We are importing demo data!', 'tokoo-vitamins' ); ?></h2>
					<p><?php esc_html_e( 'The process may take a while, since we\' importing large images', 'tokoo-vitamins' ); ?></p>
				</div>
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook ) {
			wp_enqueue_script( 'ocdi-image-picker', PT_OCDI_URL . 'assets/js/image-picker.min.js' , array( 'jquery' ), PT_OCDI_VERSION );
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'tokoo-vitamins' ),
					),
				)
			);
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'tokoo-vitamins' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					OCDI_Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'tokoo-vitamins' )
					);
				}
				...
				$log_added = OCDI_Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'tokoo-vitamins' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'tokoo-vitamins' ) );
			}
		}
		...
		/**
		 * 6. Import theme options.
		 */
		if ( ! empty( $this->selected_import_files['toptions'] ) && empty( $this->frontend_error_messages ) ) {
			$this->import_theme_options( $this->selected_import_files['toptions'] );
		}

		/**
		 * 7. After import setup.
		 */
		...
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly', 'tokoo-vitamins' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'tokoo-vitamins' ),
	...
	private function import_content( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			...
			// Add this message to log file.
			$log_added = OCDI_Helpers::append_to_file(
				$message . PHP_EOL . esc_html__( 'Max execution time after content import = ' , 'tokoo-vitamins' ) . ini_get( 'max_execution_time' ),
				$this->log_file_path,
				esc_html__( 'Importing content' , 'tokoo-vitamins' )
			);
		}
	...
	private function import_widgets( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', 'tokoo-vitamins' )
			);
		}
	...
	private function import_customizer( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', 'tokoo-vitamins' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Customizer settings import finished!', 'tokoo-vitamins' ),
			$this->log_file_path,
			esc_html__( 'Importing customizer settings' , 'tokoo-vitamins' )
		);
	}
	...
	/**
	 * Import customizer from a txt file, generated by the Codestart Export/Import.
	 *
	 * @param string $theme_options_import_file_path path to the customizer import file.
	 */
	private function import_theme_options( $theme_options_import_file_path ) {

		// Try to import the customizer settings.
		$results = OCDI_Toptions_Importer::import_theme_options( $theme_options_import_file_path );
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing options settings', 'tokoo-vitamins' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Options settings import finished!', 'tokoo-vitamins' ),
			$this->log_file_path,
			esc_html__( 'Importing options settings' , 'tokoo-vitamins' )
		);
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-ocdi/time_for_one_ajax_call', 25 ) ) {
			...
			$log_added = OCDI_Helpers::append_to_file(
				__( 'Completed AJAX call number: ' , 'tokoo-vitamins' ) . $this->ajax_call_number . PHP_EOL . $message,
				$this->log_file_path,
				''
			);
	 ...
	 * @since 1.0
	 */
	public function get_importer_configs() {
		$configs = array();
		return apply_filters( 'tokoo_importer_configs', $configs );
	}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = OCDI_Helpers::validate_import_file_info( apply_filters( 'tokoo_importer_configs', array() ) );
