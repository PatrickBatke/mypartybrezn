...
class Tokoo_Instagram_Photo_Slides extends Tokoo_Widget {

	function __construct() {

		$args = array(
			'label' 		=> __( 'Tokoo - Instagram Photo Slides', 'tokoo' ),
			'description' 	=> __( 'A custom widget to display instagram photos in slides display.', 'tokoo' ),
		 );
		...
		$args['fields'] = array(
			...
			array(
				...
				'type' 		=> 'text',
				'class' 	=> 'widefat',
				'std' 		=> __( 'Instagram Photo Slides', 'tokoo' ),
				'validate' 	=> 'alpha_dash',
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['username'] != '' ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				$count 		= 1;
				$liclass 	= esc_attr( apply_filters( 'tokoo_item_class', '' ) );
				$aclass 	= esc_attr( apply_filters( 'tokoo_a_class', '' ) );
				$imgclass 	= esc_attr( apply_filters( 'tokoo_img_class', '' ) );

				echo '<div class="koo-image-slider">';
					echo '<ul class="slides">';
					foreach ( $media_array as $item ) {
						$style = 'width='.$instance['t_width'].' height='.$instance['t_height'];
						echo '<li><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $instance['target'] ) .'"  class="'. $aclass .'"><img src="'. esc_url( $item['thumbnail'] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. $imgclass .'" '.$style.' ></a></li>';
					}
					echo '</ul>';
				echo '</div>';
			}
