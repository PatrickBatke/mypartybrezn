	...
	public function __construct ( $meta_box ) {
		...
		if ( isset( $meta_box['use_with_theme'] ) ) {
			if ( $meta_box['use_with_theme'] === true ) {
				$this->SelfPath = get_template_directory_uri() . '/taxonomy-metabox';
	...
	public function show_field_repeater( $field, $meta ) {
		...
		$c = 0;
			...
			if ( count( $meta ) > 0 && is_array( $meta ) ) {
				...
				echo '" alt="'.__( 'Remove',  'tokoo-vitamins' ).'" title="'.__( 'Remove', 'tokoo-vitamins' ).'" id="remove-'.$field['id'].'"></div>';
				$c = $c + 1;
		...
		echo '" alt="'.__( 'Add', 'tokoo-vitamins' ).'" title="'.__( 'Add', 'tokoo-vitamins' ).'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_color( $field, $meta ) {
		...
		//  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . __( 'Select a color' ) . "</a>";
			echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'tokoo-vitamins' ) . "'/>";
