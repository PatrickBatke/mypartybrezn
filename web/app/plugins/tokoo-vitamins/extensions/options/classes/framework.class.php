...
class CSFramework {
	...
	public function __construct() {
		...
		$this->settings = apply_filters( 'tokoo_framework_settings', $settings );
		$this->options  = apply_filters( 'tokoo_framework_options', $options );
		...
		if( ! empty( $this->options ) ) {
			...
			add_action( 'wp_ajax_cs-export-options', array( $this, 'export' ) );
	...
	public function validate_save( $request ) {
		...
		if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
			...
			$add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'tokoo-vitamins' ), 'updated' );
		}

		// reset all options
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'tokoo-vitamins' ), 'updated' );
			return;
		}
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'tokoo-vitamins' ), 'updated' );
		}
	...
	public function admin_page() {
		...
		echo '<div class="cs-framework cs-option-framework">';
			...
			echo '<h1>Theme Options <small>by Tokoo</small></h1>';
			echo '<fieldset>';
			echo ( $this->settings['ajax_save'] === true ) ? '<span id="cs-save-ajax">'. __( 'Settings saved.', 'tokoo-vitamins' ) .'</span>' : '';
			submit_button( __( 'Save', 'tokoo-vitamins' ), 'primary', 'save', false, array( 'data-ajax' => $this->settings['ajax_save'], 'data-save' => __( 'Saving...', 'tokoo-vitamins' ) ) );
			submit_button( __( 'Restore', 'tokoo-vitamins' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
			echo '</fieldset>';
			echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. __( 'show all options', 'tokoo-vitamins' ) .'</a>' : '';
	...
	public function export() {
		...
		echo cs_encode_string( get_option( $this->unique ) );
