...
if ( ! class_exists( 'Niku_Envato_Theme_Update' ) ) {
	/**
	 * @author Toan Nguyen
	 * @Class:  Niku_Envato_Theme_Update
	 * @Version: 1.1
	...
	class Niku_Envato_Theme_Update {
		...
		function __construct( $slug, $purchase_code = '', $access_token = '', $debug = false ) {
			...
			add_action( 'wp_ajax_oilumeo_view_changelogs' , array( $this , 'get_changelogs'  ) );
		}
		...
		function get_remote_data( $apiurl ) {
			...
			$args 		= apply_filters( 'oilumeo_get_remote_data' , $args );
		...
		function check_for_update( $transient ){
			...
			if ( $this->remote_theme_name == $current_theme_name && version_compare( $this->remote_theme_version, $current_theme_version, '>' ) ) {
				...
				$data['url']			=	esc_url( admin_url( 'admin-ajax.php' ) ) . '?action=oilumeo_view_changelogs&theme=' . $this->remote_theme_name;
