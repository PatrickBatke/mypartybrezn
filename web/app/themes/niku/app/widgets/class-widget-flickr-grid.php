...
class Tokoo_Flickr_Photo_Grid extends Tokoo_Widget {
	...
	function __construct() {
		...
		$args = array(
			'label' 		=> esc_html__( 'Tokoo - Flickr Photo Grid', 'tokoo' ),
			'description' 	=> esc_html__( 'A custom widget to display the flickr photo in grid display.', 'tokoo' ),
		 );
		...
		$args['fields'] = array(
			...
			array(
				...
				'type' 		=> 'text',
				'class' 	=> 'widefat',
				'std' 		=> esc_html__( 'Flickr Photo Grid', 'tokoo' ),
				'validate' 	=> 'alpha_dash',
	...
	function get_photos( $id, $count = 8 ) {
		...
		$transient_key 	= md5( 'tokoo_flickr_grid_cache_' . $id . $count );
