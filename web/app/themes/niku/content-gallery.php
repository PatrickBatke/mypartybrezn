				...
				if ( $attachments ) { ?>
					...
					<ul class="slides">
						<?php foreach ( $attachments as $key => $attachment ) { ?>
							...
							<li><a href="<?php echo esc_url( $large_image[0] ); ?>"><?php echo wp_get_attachment_image( $attachment->ID, 'blog_thumbnail', false, false ); ?></a></li>
						<?php } ?>
					</ul><!-- .tile-layout -->
		...
		<div class="inner-post">
			...
			<div class="entry-details">
				...
				<div class="entry-content">
					...
					<?php if ( $attachments ) : ?>
						<?php $count = count( $attachments ); // show gallery count ?>
						<p class="image-count"><?php printf( _n( 'This gallery contains %d item.', 'This gallery contains %d items.', $count, 'tokoo' ), $count ); ?></p>
					<?php endif ?>
			...
			<?php else : ?>
				...
				<div class="entry-excerpt">
					<?php
						...
						$attachments_all = get_children( $args );
					?>
					<?php the_excerpt(); ?>
					<?php wp_link_pages( array( 'before' => '<div class="article-paging">' . '<strong>' . esc_html__( 'Pages:', 'tokoo' ) . '</strong>', 'after' => '</div>' ) ); ?>
				</div><!-- .entry-details -->
